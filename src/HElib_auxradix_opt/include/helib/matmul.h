...
public:
  ...
  ConstMultiplierCache directCache;
  ConstMultiplierCache directCache1; // direct diagonals for LC-AKS experiments
  ...
  void upgrade() override
  {
    ...
    directCache.upgrade(ea.getContext());
    directCache1.upgrade(ea.getContext());
  }

  const EncryptedArray& getEA() const override { return ea; }
};

// A 1D linear transformation executor for sparse diagonal matrices.
// This is intended for experimental factored EvalMap stages: it evaluates
// only the diagonal offsets supplied by the caller instead of treating the
// transform as a dense D-dimensional matrix.
class SparseMatMul1DExec : public MatMulExecBase
{
public:
  const EncryptedArray& ea;
  ...
  SparseMatMul1DExec(const MatMul1D& mat, const std::vector<long>& offsets);
