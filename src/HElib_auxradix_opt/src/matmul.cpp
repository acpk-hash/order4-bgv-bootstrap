...
static bool lcAksFusedDigitsCoeffToSlotEnabled()
{
  const char* flag = std::getenv("HELIB_AUX_LC_AKS_FUSED_DIGITS");
...
public:
  BasicAutomorphPrecon(const Ctxt& _ctxt) : ctxt(_ctxt), noise(1.0)
  {
    ...
    // Keep nonzero-key ciphertexts under their current key.  The delayed
    // LC-AKS experiment needs sparse-key baby-step hoisting before switching
    // the giant-step accumulators back to key 0.
    if (keyID == 0) {
      ctxt.cleanUp();
    } else {
      ctxt.reLinearize(keyID);
      if (!ctxt.primeSet.disjointFrom(context.getSpecialPrimes()) ||
          !ctxt.primeSet.disjointFrom(context.getSmallPrimes())) {
...
static const ConstMultiplier_DoubleCRT* asDCRTConstMultiplier(
    const std::shared_ptr<ConstMultiplier>& ptr)
{
  if (!ptr)
    return nullptr;
  return dynamic_cast<const ConstMultiplier_DoubleCRT*>(ptr.get());
}
...
template <typename type>
struct SparseMatMul1DExec_construct
{
  PA_INJECT(type)
                    ...
                    const std::vector<long>& offsets)
  {
    ...
    vec.resize(offsets.size());
    if (!native)
      vec1.resize(offsets.size());

    for (long pos : range((long)offsets.size())) {
      long i = offsets[pos];
      ...
      if (native) {
        vec[pos] = build_ConstMultiplier(poly, dim, 0, ea);
      } else {
        if (IsZero(poly)) {
          vec[pos] = nullptr;
          vec1[pos] = nullptr;
        ...
        vec[pos] = build_ConstMultiplier(poly1, dim, 0, ea);
        vec1[pos] = build_ConstMultiplier(poly2, dim, D, ea);
      }
    }
  }
};

HELIB_NO_CKKS_IMPL(SparseMatMul1DExec_construct)

SparseMatMul1DExec::SparseMatMul1DExec(const MatMul1D& mat,
                                       const std::vector<long>& raw_offsets) :
    ea(mat.getEA())
{
  HELIB_NTIMER_START(SparseMatMul1DExec);
  ...
  for (long& offset : offsets)
    offset = mcMod(offset, D);
  std::sort(offsets.begin(), offsets.end());
  offsets.erase(std::unique(offsets.begin(), offsets.end()), offsets.end());

  ea.dispatch<SparseMatMul1DExec_construct>(mat,
                                            cache.multiplier,
                                            cache1.multiplier,
                                            offsets);
}

void SparseMatMul1DExec::mul(Ctxt& ctxt) const
{
  HELIB_NTIMER_START(mul_SparseMatMul1DExec);
  ...
  if (isSetAutomorphVals()) {
    for (long offset : offsets)
      recordAutomorphVal(zMStar.genToPow(dim, offset));
    if (!native)
      recordAutomorphVal(zMStar.genToPow(dim, -D));
    return;
  }
  ...
  for (long pos : range((long)offsets.size())) {
    if (!cache.multiplier[pos] &&
        (native || !cache1.multiplier[pos]))
      continue;

    std::shared_ptr<Ctxt> tmp = precon->automorph(offsets[pos]);
    if (native) {
      DestMulAdd(acc, cache.multiplier[pos], *tmp);
    } else {
      MulAdd(acc, cache.multiplier[pos], *tmp);
      DestMulAdd(acc1, cache1.multiplier[pos], *tmp);
    }
  }
    ...
    ea(mat.getEA()), minimal(_minimal)
{
  ...
  if (ea.getTag() == PA_cx_tag) {
    ...
    if (lcAksDelayedCoeffToSlotEnabled())
      MatMul1DExec_construct_CKKS(ea.getCx(), mat, directCache.multiplier, 0);
  } else {
    ...
    if (lcAksDelayedCoeffToSlotEnabled())
      ea.dispatch<MatMul1DExec_construct>(mat,
                                          directCache.multiplier,
                                          directCache1.multiplier,
                                          0);
  }
}
...
void MatMul1DExec::mul(Ctxt& ctxt) const
{
  ...
  if (lcAksFusedDigitsCoeffToSlotEnabled() && ctxt.getKeyID() != 0 &&
      g != 0 && native) {
    bool allConstantsDCRT = true;
    for (long i : range(D)) {
      if (cache.multiplier[i] && !asDCRTConstMultiplier(cache.multiplier[i])) {
        allConstantsDCRT = false;
        break;
      }
    }

    if (allConstantsDCRT) {
      HELIB_NTIMER_START(mul_MatMul1DExec_LC_AKS_FUSED_DIGITS);

      const Context& context = ctxt.getContext();
      const PubKey& pubKey = ctxt.getPubKey();
      const long keyID = ctxt.getKeyID();
      const IndexSet specialPrimes = context.getSpecialPrimes();
      const IndexSet targetPrimeSet = ctxt.getPrimeSet() | specialPrimes;
      const double logProd = context.logOfProduct(specialPrimes);
      ...
      acc[index].primeSet = targetPrimeSet;
      acc[index].ptxtSpace = ctxt.ptxtSpace;
      acc[index].intFactor = ctxt.intFactor;
      acc[index].ptxtMag = ctxt.ptxtMag;
      acc[index].ratFactor = ctxt.ratFactor * NTL::xexp(logProd);

      for (long k : range(first, last)) {
        const long giantAut = zMStar.genToPow(dim, g * k);
        const KeySwitch& W =
            pubKey.getKeySWmatrix(SKHandle(1, giantAut, keyID), 0);
        assertTrue(W.toKeyID >= 0, "No LC-AKS fused key-switch matrix exists");

        if (acc[index].ptxtSpace > W.ptxtSpace)
          acc[index].reducePtxtSpace(W.ptxtSpace);

        DoubleCRT weightedConst(context, ctxt.getPrimeSet());
        DoubleCRT weightedPart1(context, ctxt.getPrimeSet());
        bool hasConst = false;
        bool hasPart1 = false;
        NTL::xdouble groupNoise = NTL::to_xdouble(0.0);
        ...
        for (long j : range(g)) {
          ...
          const ConstMultiplier_DoubleCRT* mult =
              asDCRTConstMultiplier(cache.multiplier[i]);
          if (!mult)
            continue;

          if (baby_steps[j] && !baby_steps[j]->parts.empty()) {
            DoubleCRT tmp = baby_steps[j]->parts[0];
            tmp.Mul(mult->data, /*matchIndexSets=*/false);
            weightedConst.Add(tmp, /*matchIndexSets=*/false);
            hasConst = true;
            groupNoise += baby_steps[j]->noiseBound * mult->sz;
          }

          if (baby_steps[j] && baby_steps[j]->parts.size() > 1) {
            DoubleCRT tmp = baby_steps[j]->parts[1];
            tmp.Mul(mult->data, /*matchIndexSets=*/false);
            weightedPart1.Add(tmp, /*matchIndexSets=*/false);
            hasPart1 = true;
          }
        }

        if (!hasConst && !hasPart1)
          continue;

        if (k > 0) {
          if (hasConst)
            weightedConst.automorph(giantAut);
          if (hasPart1)
            weightedPart1.automorph(giantAut);
        }

        if (hasConst) {
          weightedConst.addPrimesAndScale(specialPrimes);
          acc[index].addPart(weightedConst,
                             SKHandle(),
                             /*matchPrimeSet=*/true);
          acc[index].noiseBound += groupNoise * NTL::xexp(logProd);
        }

        if (hasPart1) {
          std::vector<DoubleCRT> weightedDigits;
          NTL::xdouble addedNoise =
              weightedPart1.breakIntoDigits(weightedDigits);
          acc[index].keySwitchDigits(W, weightedDigits);
          acc[index].noiseBound += addedNoise * W.noiseBound;
        }
      }
      NTL_EXEC_INDEX_END
      ...
      HELIB_NTIMER_STOP(mul_MatMul1DExec_LC_AKS_FUSED_DIGITS);
      return;
    }

    Warning("LC-AKS fused-digits path skipped because constants were not "
            "upgraded to DoubleCRT");
  }

  if (lcAksBsgsCoeffToSlotEnabled() && ctxt.getKeyID() != 0 && g != 0 &&
      native) {
    HELIB_NTIMER_START(mul_MatMul1DExec_LC_AKS_BSGS);
    ...
    for (long k : range(first, last)) {
      ...
      if (acc_inner.isEmpty())
        continue;

      if (k > 0)
        acc_inner.automorph(zMStar.genToPow(dim, g * k));

      acc_inner.reLinearizeInto(acc[index], 0);
    }
    NTL_EXEC_INDEX_END
    ...
    HELIB_NTIMER_STOP(mul_MatMul1DExec_LC_AKS_BSGS);
    return;
  }

  if (lcAksDelayedCoeffToSlotEnabled() && ctxt.getKeyID() != 0) {
    HELIB_NTIMER_START(mul_MatMul1DExec_LC_AKS_DIRECT);
    const auto& diagMultiplier =
        directCache.multiplier.empty() ? cache.multiplier
                                       : directCache.multiplier;
    const auto& diagMultiplier1 =
        directCache1.multiplier.empty() ? cache1.multiplier
                                        : directCache1.multiplier;

    // Experimental sparse-to-dense direct path, inspired by LCR+AKS.  The
    // input is still under the sparse boot key.  For each diagonal, apply the
    // required automorphism on that sparse-key ciphertext, then immediately
    // key-switch the transformed part to the normal key using direct
    // s_boot(X^k)->s_0 matrices generated by the benchmark driver.
    //
    // This intentionally bypasses HElib's BSGS executor for this first stage:
    // it is a correctness and timing probe for fusing sparse-to-dense
    // key-switching into the first linear transform.
    const Ctxt input(ctxt);
    Ctxt acc(ZeroCtxtLike, ctxt);
    Ctxt acc1(ZeroCtxtLike, ctxt);

    for (long i : range(D)) {
      if (!diagMultiplier[i] && (native || !diagMultiplier1[i]))
        continue;

      Ctxt tmp(input);
      const long aut = zMStar.genToPow(dim, i);
      if (aut != 1)
        tmp.automorph(aut);
      tmp.reLinearize(0);
      tmp.cleanUp();

      MulAdd(acc, diagMultiplier[i], tmp);
      if (!native)
        MulAdd(acc1, diagMultiplier1[i], tmp);
    }
    ...
    ctxt = acc;
    HELIB_NTIMER_STOP(mul_MatMul1DExec_LC_AKS_DIRECT);
    return;
  }
