                            ...
                            bool toNewBootKey)
{
  ...
  DoubleCRT toKey = sKeys.at(toIdx);
  ...
  IndexSet ksPrimes = newCtxtPrimes | newSpecialPrimes;
  if (context.getNewKSFlag() && fromIdx == recryptKeyID &&
      toIdx == recryptKeyID && !toNewBootKey) {
    // The encapsulated boot key is stored on qKS+R, but the delayed
    // LC-AKS experiment needs ordinary automorphism keys for ciphertexts
    // that have already been modded up/down to the normal ciphertext
    // prime set.  Materialize both secret-key polynomials on the
    // key-switch matrix prime set before generating RLWE samples.
    fromKey.setPrimes(ksPrimes);
    toKey.setPrimes(ksPrimes);
  } else {
