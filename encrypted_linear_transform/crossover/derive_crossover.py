#!/usr/bin/env python3
"""Derive pure mixed-radix DIF butterfly stages for the length-D DFT over F_p
(scalar constants; zeta_D in F_p since D | p-1). No Rader wrap needed: D smooth.

Chain convention (same stage algebra as ../derive_butterfly.py):
  stage with block length L, radix r, m2 = L/r:
    out[b+j+i*m2] = sum_i2 in[b+j+i2*m2] * w_r^(i*i2) * w_L^(i*j)
  w_r = zeta^(D/r), w_L = zeta^(D/L).
Chain product == row-permuted DFT: chain_row[t] == W[perm[t]], W[i][j]=zeta^(ij).
Verified below on the full matrix; exports stages<D>.txt for enc_crossover.cpp.
"""
import sys
from sympy import isprime, primitive_root

def derive(D, p, radices, outfile):
    assert isprime(p) and (p-1) % D == 0
    prod = 1
    ...
    # verify chain == permuted DFT (apply chain to unit vectors -> columns)
    cols = []
    for j in range(D):
        v = [0]*D; v[j] = 1
        for M in stages:
            ...
            v = out
        cols.append(v)
    # chain matrix row t = [cols[j][t] for j]
    Wrow_index = {}
    for i in range(D):
